...
/** This class manages the buying and selling of stocks */
public class Stock extends Investment {
  ...
  // copy constructor
  public Stock(Stock stockInvestment) {
    this.setSymbol(stockInvestment.getSymbol());
    this.setName(stockInvestment.getName());
    this.setQuantity(stockInvestment.getQuantity());
    this.setPrice(stockInvestment.getPrice());
    this.setBookValue(stockInvestment.getQuantity() * stockInvestment.getPrice() + 9.99);
  }

  /** This is a method to buy a stock */
  public static void Buy(Investment investments, String investmentSymbol, GUI ePortfolio) {
    ...
    for (int k = 0; k < investments.investmentList.size(); k++) {
      while (investmentSymbol.compareTo(investments.investmentList.get(k).getSymbol()) == 0
          && investments.investmentList.get(k) instanceof MutualFund) {
        ePortfolio.messageBuy.setText(ePortfolio.messageBuy.getText()
            + "That symbol is already taken for a mutual fund investment, please enter a different symbol for your investment:");
        return;
      }
    }
    ...
    for (int i = 0; i < investments.investmentList.size(); i++) {
      ...
      if (investments.investmentList.get(i).getSymbol().compareTo(investmentSymbol) == 0
          && investments.investmentList.get(i) instanceof Stock) {
        symbolExists = 1;
      }
      index = i;
    }
    ...
    if (symbolExists == 1) {
      ...
      if (Integer.parseInt(ePortfolio.buyQuantityField.getText()) < 0) {
      ...
      try {
        ...
        ePortfolio.messageBuy.setText("Please enter a whole number and not a string or a double");
        return;
      }
      ...
      investments.investmentList.get(index).setBookValue(investments.investmentList.get(index).getBookValue()
          + quantity * investments.investmentList.get(index).getPrice() + 9.99);
    } else {
      ...
      try {
        investments.investmentList.add(new Stock(investmentSymbol, name, quantity, price));
      } catch (Exception e) {
    ...
    ePortfolio.messageBuy.setText("Stock bought successfully");
  }

  /** This is a method to sell a stock */
    ...
    for (int i = 0; i < investments.investmentList.size(); i++) {
      if (investmentSymbol.compareTo(investments.investmentList.get(i).getSymbol()) == 0) {
        if (investments.investmentList.get(i).getQuantity() < quantity) {
          ...
          ePortfolio.messageSell.setText("You recieved $" + (quantity * price - 9.99) + " from the sale");
        } else {
